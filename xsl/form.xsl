...
<body>
	<xsl:choose>
		<xsl:when test="$pagetype = 'onecol' or $pagetype = 'form'">
...
<xsl:template name="standard-interior">
	<div>
		...
		<xsl:choose>
			<xsl:when test="$pagetype ='form'">
				<xsl:choose>
					<xsl:when test="$ou:action = 'pub'">
						<xsl:processing-instruction name="php"> 
							require("/opt/machform/machform/machform.php");
							$mf_param['form_id'] = <xsl:value-of select="document/config/parameter[@name='formid']/option[@selected = 'true']/@value"/>;
							$mf_param['base_path'] = 'https://www.malone.edu/forms/machforms/';
							display_machform($mf_param);
						</xsl:processing-instruction>
					</xsl:when>
					<xsl:otherwise>
						<h5>Web Form</h5>
						<div class="pr_content_preview">
						<p>This page is a place holder for a web form NOTE: When the page is published this message will be replaced with the appropriate form on the live site.<br />
						To edit your forms, log into the <a target="_blank" href="https://www.malone.edu/forms/machforms/">MachForm tool.</a></p>
						</div>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
